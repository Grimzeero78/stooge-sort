...
step 1: compare first and last element ensuring the smallest btw the two comes on index 0.**if(arr[l] > arr[h]) swap(arr[l], arr[h]);**
step 2:Checks if the array has 2 or more elements if yes the recursive sorting will continue if otherwise its done and sorted.**if(h - l + 1 > 2)**
step 3:claculate 1/3 of the section of the array **int t = (h - l + 1) / 3;**
